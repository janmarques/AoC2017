...
void Init(string reg) => registers.TryAdd(reg, 0);
foreach (var line in input.Split(Environment.NewLine))
{
    var split = line.Split(' ');
    var a = split[0];
    var aOp = split[1];
    var aOpVal = int.Parse(split[2]);
    var b = split[4];
    var bOp = split[5];
    var bOpVal = int.Parse(split[6]);
    ...
    bool Check()
    {
        var bRegVal = registers[b];
        if (bOp == ">") { return bRegVal > bOpVal; }
        if (bOp == ">=") { return bRegVal >= bOpVal; }
        if (bOp == "<") { return bRegVal < bOpVal; }
        if (bOp == "<=") { return bRegVal <= bOpVal; }
        if (bOp == "==") { return bRegVal == bOpVal; }
